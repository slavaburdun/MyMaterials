...
public class Log4jInit extends HttpServlet {
    ...
    public void init() {
        ...
        Logger globallog = Logger.getLogger("LOGFILE");
        getServletContext().setAttribute("log4", globallog);
        getServletContext().setAttribute("logfilename", logfilename);
        Logger dbLogger = Logger.getLogger("DBLOGFILE");
        getServletContext().setAttribute("log4db", dbLogger);
        globallog.info("Load-onstart-up Servlet");
        dbLogger.info("Load-onstart-up Servlet DB logger");
    }
